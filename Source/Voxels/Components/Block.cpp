...
void Block::CreateQuad(BlockFace face, BlockType type,
  ...
  unsigned short idx =
      procModel->indices_.Size() ? procModel->indices_.Back() + 1 : 0;
  procModel->indices_.Push(
      {idx, (unsigned short)(idx + 1), (unsigned short)(idx + 2), idx,
       (unsigned short)(idx + 2), (unsigned short)(idx + 3)});

  procModel->uvs_.Push(
      {UVs[type][0], UVs[type][1], UVs[type][2], UVs[type][3]});

  switch (face) {
    case BACK:
      procModel->positions_.Push({points[0], points[1], points[2], points[3]});
      procModel->normals_.Push(
    ...
    case FRONT:
      ...
      procModel->normals_.Push({Vector3::FORWARD, Vector3::FORWARD,
