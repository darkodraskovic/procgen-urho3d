...
void ShaderToy::Start() {
  ...
  ProcGen::MaterialCreator* materialCreator =
      GetSubsystem<ProcGen::MaterialCreator>();
  ...
  // diffuseTexture = textureCreator->CreateEffectTexture(w, h,
  // "PPE_ScottishTartan"); diffuseTexture =
  // textureCreator->CreateEffectTexture(w, h, "PPE_Patterns_TicTacToe");
  ...
  HashMap<TextureUnit, Texture*> textureData = {
      {TU_DIFFUSE, diffuseTexture},
      {TU_NORMAL, normalTexture},
      {TU_ENVIRONMENT, skyboxTexture},
  };
  ...
  // node = modelCreator->CreateStockModel("Plane", material, Vector3::ZERO,
  // Quaternion(90, 0, 0));
