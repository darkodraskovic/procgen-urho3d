...
void CharacterController::RegisterObject(Context* context) {
  ...
  // load / save handling. We specify the Default attribute mode which means it
  // will be used both for saving into file, and network replication
...
// void CharacterController::OnSceneSet(Scene* scene)
{
  ...
  SubscribeToEvent(GetNode(), E_NODECOLLISION,
                   URHO3D_HANDLER(CharacterController, HandleNodeCollision));
}
...
void CharacterController::FixedUpdate(float timeStep) {
  /// \todo Could cache the components for faster access instead of finding them
  /// each frame
...
void CharacterController::HandleNodeCollision(StringHash eventType,
                                              VariantMap& eventData) {
  // Check collision contacts and see if character is standing on ground (look
  // for a contact that has near vertical normal)
  ...
  while (!contacts.IsEof()) {
    ...
    // If contact is below node center and pointing up, assume it's a ground
    // contact
    if (contactPosition.y_ <
        (node_->GetPosition().y_ + 1.0f))  // below node center
    {
